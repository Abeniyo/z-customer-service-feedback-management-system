...
const StatsSummary = ({ focals }) => {
  // Calculate stats based on actual focal data
  const activeFocals = focals.filter(f => f.status === 'active').length;
  const totalCustomers = focals.reduce((acc, f) => acc + (f.customer_count || 0), 0);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold">{focals.length}</p>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-green-600">{activeFocals}</p>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500">Avg Customers/Focal</p>
            <p className="text-2xl font-bold text-yellow-600">
              {focals.length ? (totalCustomers / focals.length).toFixed(1) : '0'}
            </p>
          </div>
...
const FocalCard = ({ 
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(focal.status)}`}>
                  {focal.status === 'active' ? 'Active' : 'Inactive'}
                </span>
              </div>
              <p className="text-sm text-gray-500">
                {focal.branch_name || 'No Branch'} • {focal.phone_number || 'No Phone'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p className="font-semibold">{customers?.length || 0}</p>
              </div>
            </div>
      ...
      {/* Expanded Customer List - UPDATED to match API response */}
      {isExpanded && (
...
const FocalManagement = () => {
  ...
  const fetchFocals = async () => {
    ...
    setError(null);
    try {
      const response = await api.get('/focal/focals/');
      ...
      // Store focal names for reference
      const names = {};
      focalData.forEach(focal => {
        const fullName = [focal.first_name, focal.middle_name, focal.last_name]
          .filter(Boolean)
          .join(' ');
        names[focal.id] = fullName;
      });
      setFocalNames(names);
  ...
  // Fetch customers for a specific focal - UPDATED to match API response
  const fetchFocalCustomers = async (focalId) => {
    ...
    try {
      ...
      // API returns { focal: "name", customers: [...] }
  ...
  const filteredFocals = focals.filter(focal => {
    ...
    const matchesSearch = fullName.includes(searchTerm.toLowerCase()) ||
                         (focal.branch_name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                         (focal.email || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                         (focal.phone_number || '').includes(searchTerm);
    
    const matchesStatus = filterStatus === 'all' || focal.status === filterStatus;
  ...
  // Customer Modal Component - UPDATED for full_name field
  const CustomerModal = () => {
  ...
  const AssignModal = () => {
    ...
    const fetchAvailableCustomers = async () => {
      setLoading(true);
      try {
        // Get customers not assigned to any focal
        const response = await api.get('/customers/?unassigned=true');
    ...
    const handleAssign = async () => {
      ...
      try {
        ...
        // Refresh customers for this focal if it's expanded
    ...
    const filteredCustomers = availableCustomers.filter(customer => {
      const fullName = (customer.full_name || 
        `${customer.first_name || ''} ${customer.middle_name || ''} ${customer.last_name || ''}`).toLowerCase();
